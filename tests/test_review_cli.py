...
"""
    ...
    monkeypatch.setattr("pipeline.review.cli.collect_diff", fake_collect)
...
def test_main_check_filter_passed_to_run_checks(monkeypatch):
    ...
    def fake_run_checks(diffs, enabled=None, ignored=None, max_line_length=None):
        ...
        from pipeline.review.models import Review
        return Review()

    monkeypatch.setattr("pipeline.review.cli.run_checks", fake_run_checks)
...
def test_main_json_format_calls_format_json(monkeypatch):
    ...
    monkeypatch.setattr("pipeline.review.cli.format_text", fake_format_text)
    monkeypatch.setattr("pipeline.review.cli.format_json", fake_format_json)
